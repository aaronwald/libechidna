...
// ============================================================================
// Additional Defensive / Aggressive Tests
// ============================================================================

// LogWriteBuf::Backup edge cases
TEST(LogWriteBufTest, BackupZero)
{
  LogWriteBuf<MMapAnon> store(MemManager::GetPageSize(), 0, -1, false, true);

  void *data = nullptr;
  int len = 0;
  ASSERT_EQ(store.ZeroCopyNext(&data, &len), 0);

  // Backup(0) should be valid (no-op, stays at page-full position)
  EXPECT_TRUE(store.Backup(0));
}

TEST(LogWriteBufTest, BackupOverBudget)
{
  LogWriteBuf<MMapAnon> store(MemManager::GetPageSize(), 0, -1, false, true);

  // Push a small amount so _dataPage.second == 5
  ASSERT_EQ(store.Push("hello", 5), 0);

  // Trying to back up more than written should fail
  EXPECT_FALSE(store.Backup(6));
}

TEST(LogWriteBufTest, BackupExactPosition)
{
  LogWriteBuf<MMapAnon> store(MemManager::GetPageSize(), 0, -1, false, true);

  ASSERT_EQ(store.Push("hello", 5), 0);

  // Backup exactly what was written (back to offset 0 on the page)
  EXPECT_TRUE(store.Backup(5));

  // Should still be able to write since the page is allocated but at offset 0
  EXPECT_EQ(store.Push("world", 5), 0);
}

// LogWriteBuf: push zero-length is a no-op
TEST(LogWriteBufTest, PushZeroLength)
{
  LogWriteBuf<MMapAnon> store(MemManager::GetPageSize(), 0, -1, false, true);

  // Should succeed without allocating a page (len==0 exits the while loop immediately)
  EXPECT_EQ(store.Push("ignored", 0), 0);
}

// LogRWStream: Peak at exactly _available boundary is out-of-range
TEST(LogRWStreamTest, PeakAtBoundary)
{
  LogRWStream<MMapAnon, OneShotCache, 16> rwBuf(MemManager::GetPageSize(), 0, -1, true);

  ASSERT_EQ(rwBuf.Push("abcd", 4), 0);

  char d = 0;
  // offset 3 is last valid byte
  EXPECT_TRUE(rwBuf.Peak(3, d));
  EXPECT_EQ(d, 'd');

  // offset == _available (4) is out of range
  EXPECT_FALSE(rwBuf.Peak(4, d));

  // offset > _available is also out of range
  EXPECT_FALSE(rwBuf.Peak(100, d));
}

// LogRWStream: Find sets offset to UINT64_MAX when character is not found
TEST(LogRWStreamTest, FindNotFoundSetsUINT64MAX)
{
  LogRWStream<MMapAnon, OneShotCache, 16> rwBuf(MemManager::GetPageSize(), 0, -1, true);

  ASSERT_EQ(rwBuf.Push("abcdef", 6), 0);

  uint64_t offset = 42; // sentinel
  EXPECT_FALSE(rwBuf.Find(0, 'z', offset));
  EXPECT_EQ(offset, UINT64_MAX);
}

// LogRWStream: Pop is byte-bounded by _available (consistent with Peak / ZeroCopyReadNext)
TEST(LogRWStreamTest, PopBeyondAvailableReadsZeroedBytes)
{
  LogRWStream<MMapAnon, OneShotCache, 16> rwBuf(MemManager::GetPageSize(), 0, -1, true);

  ASSERT_EQ(rwBuf.Push("hello", 5), 0);
  ASSERT_EQ(rwBuf.Available(), 5u);

  char dest[10] = {};
  // start_offset >= _available → false
  EXPECT_FALSE(rwBuf.Pop(5, dest, 1));
  EXPECT_FALSE(rwBuf.Pop(100, dest, 1));

  // start_offset + size > _available → false (would read zeroed mmap bytes)
  EXPECT_FALSE(rwBuf.Pop(0, dest, 10));
  EXPECT_FALSE(rwBuf.Pop(3, dest, 3));

  // Exactly up to _available → true
  EXPECT_TRUE(rwBuf.Pop(0, dest, 5));
  EXPECT_EQ(memcmp(dest, "hello", 5), 0);

  EXPECT_TRUE(rwBuf.Pop(2, dest, 3));
  EXPECT_EQ(memcmp(dest, "llo", 3), 0);
}

// LogRWStream: Readv data is readable via Pop (data integrity through Readv path)
TEST(LogRWStreamTest, ReadvDataIntegrity)
{
  LogRWStream<MMapAnon, OneShotCache, 16> rwBuf(MemManager::GetPageSize(), 0, -1, true);
  ...
  const char *testData = "readvcheck";
  ASSERT_EQ(write(fds[1], testData, 10), 10);

  int r = rwBuf.Readv(fds[0], rcb);
  EXPECT_EQ(r, 10);
  EXPECT_EQ(rwBuf.Available(), 10u);

  char dest[10] = {};
  EXPECT_TRUE(rwBuf.Pop(0, dest, 10));
  EXPECT_EQ(memcmp(dest, testData, 10), 0);

  close(fds[0]);
  close(fds[1]);
}

// LogRWStream: Writev on LRU stream when page is not in cache returns -2
TEST(LogRWStreamTest, WritevPageNotInCache)
{
  char buf[1024];
  int fd = FileUtil::MakeTemp("coypu", buf, sizeof(buf));
  ASSERT_TRUE(fd > 0);
  ...
  size_t pageSize = MemManager::GetPageSize();
  // Cache holds only 1 page; write 3 pages so the first is evicted
  LogRWStream<MMapShared, LRUCache, 1> rwBuf(pageSize, 0, fd, false);
  ...
  // Attempt to writev starting from offset 0 (page 0, evicted from 1-page LRU cache)
  // after the 3rd-page push, only page 2 (or the last accessed page) is cached.
  // Accessing page 0 should cause FindPage to load it via mmap (LRU remaps on miss).
  // Verify the return is either the data length or -2 (cache miss fallback).
  int result = rwBuf.Writev(0, pageSize, pipeFds[1], wcb);
  // LRU will remap on miss, so this should succeed
  EXPECT_GT(result, 0);
...
// OneShotCache: accessing a page after it has been evicted returns -1
TEST(OneShotCacheTest, EvictedPageReturnsNotFound)
{
  typedef OneShotCache<MMapAnon, 32> cache_type;
  size_t pageSize = MemManager::GetPageSize();
  cache_type oneShot(pageSize, 0, -1);
  ...
  char *a = reinterpret_cast<char *>(MMapAnon::MMapWrite(-1, 0, pageSize));
  char *b = reinterpret_cast<char *>(MMapAnon::MMapWrite(-1, 0, pageSize));
  ASSERT_NE(a, nullptr);
  ASSERT_NE(b, nullptr);

  oneShot.AddPage(a, 0);
  oneShot.AddPage(b, pageSize);

  // Finding page 1 (offset = pageSize) causes page 0 to be evicted (offset < pageSize)
  ASSERT_EQ(oneShot.FindPage(pageSize, page), 0);

  // Page 0 has been evicted; FindPage should return -1
  EXPECT_EQ(oneShot.FindPage(0, page), -1);
}

// PositionedStream: Backup to exactly offset 0 (from non-zero position)
TEST(PositionedStreamTest, BackupToZero)
{
  typedef LogRWStream<MMapAnon, OneShotCache, 16> stream_type;
  auto stream = std::make_shared<stream_type>(MemManager::GetPageSize(), 0, -1, true);
  ...
  ASSERT_EQ(posStream.Push("hello", 5), 0);
  ASSERT_TRUE(posStream.Skip(5));
  EXPECT_EQ(posStream.Available(), 0u);

  // Back up all the way to the beginning
  EXPECT_TRUE(posStream.Backup(5));
  EXPECT_EQ(posStream.Available(), 5u);
  EXPECT_EQ(posStream.CurrentOffset(), 0u);

  // One more byte should fail (can't go before the start)
  EXPECT_FALSE(posStream.Backup(1));
}

// PositionedStream: ZeroCopyReadNext caps *len to Available()
TEST(PositionedStreamTest, ZeroCopyReadNextLenCapped)
{
  typedef LogRWStream<MMapAnon, OneShotCache, 16> stream_type;
  auto stream = std::make_shared<stream_type>(MemManager::GetPageSize(), 0, -1, true);
  ...
  // Write only 8 bytes into a page that has capacity for the full page
  ASSERT_EQ(posStream.Push("12345678", 8), 0);

  const void *data = nullptr;
  int len = 0;

  EXPECT_TRUE(posStream.ZeroCopyReadNext(0, &data, &len));
  // len must not exceed the 8 bytes actually available
  EXPECT_LE(len, 8);
  EXPECT_GT(len, 0);
  EXPECT_EQ(memcmp(data, "12345678", len), 0);
}

// MultiPositionedStreamLog: Mark on unregistered (out-of-range) fd returns false
TEST(MultiPositionedStreamLogTest, MarkUnregisteredFd)
{
  typedef LogRWStream<MMapAnon, OneShotCache, 16> stream_type;
  auto stream = std::make_shared<stream_type>(MemManager::GetPageSize(), 0, -1, true);
  ...
  ASSERT_EQ(multiStream.Push("data", 4), 0);

  // fd 999 was never registered, so _curOffsets won't contain it
  EXPECT_FALSE(multiStream.Mark(999, 0));
}

// MultiPositionedStreamLog: MarkEnd on unregistered (out-of-range) fd returns false
TEST(MultiPositionedStreamLogTest, MarkEndUnregisteredFd)
{
  typedef LogRWStream<MMapAnon, OneShotCache, 16> stream_type;
  auto stream = std::make_shared<stream_type>(MemManager::GetPageSize(), 0, -1, true);
  ...
  EXPECT_FALSE(multiStream.MarkEnd(999));
}

// MultiPositionedStreamLog: Unregister on out-of-range fd returns error
TEST(MultiPositionedStreamLogTest, UnregisterOutOfRange)
{
  typedef LogRWStream<MMapAnon, OneShotCache, 16> stream_type;
  auto stream = std::make_shared<stream_type>(MemManager::GetPageSize(), 0, -1, true);
  ...
  // fd 999 was never registered; _curOffsets is empty → out of range
  EXPECT_EQ(multiStream.Unregister(999), -3);
}

// MultiPositionedStreamLog: Pop with offset beyond stream available returns false
TEST(MultiPositionedStreamLogTest, PopBeyondAvailable)
{
  typedef LogRWStream<MMapAnon, OneShotCache, 16> stream_type;
  auto stream = std::make_shared<stream_type>(MemManager::GetPageSize(), 0, -1, true);
  ...
  ASSERT_EQ(multiStream.Push("short", 5), 0);

  char dest[20] = {};
  // offset > available
  EXPECT_FALSE(multiStream.Pop(dest, 10, 5));
}

// LogRWStream: with maxSize the capacity is bounded
TEST(LogRWStreamTest, MaxSizeConstraint)
{
  const uint64_t maxSize = 1024;
  LogRWStream<MMapAnon, OneShotCache, 16> rwBuf(MemManager::GetPageSize(), 0, -1, true, maxSize);

  EXPECT_EQ(rwBuf.Capacity(), maxSize);
  EXPECT_EQ(rwBuf.Free(), maxSize);

  ASSERT_EQ(rwBuf.Push("hello", 5), 0);
  EXPECT_EQ(rwBuf.Available(), 5u);
  EXPECT_EQ(rwBuf.Free(), maxSize - 5);
}

// LogRWStream: iterator over zero-element range is immediately equal
TEST(LogRWStreamTest, IteratorEmptyRange)
{
  LogRWStream<MMapAnon, OneShotCache, 16> rwBuf(MemManager::GetPageSize(), 0, -1, true);

  ASSERT_EQ(rwBuf.Push("hello", 5), 0);

  auto b = rwBuf.begin(3);
  auto e = rwBuf.end(3);
  EXPECT_EQ(b, e);
...
// logstreambuf: xsputn returns Push result (0), not byte count — verify stream failbit behavior
TEST(LogStreamBufTest, XsputnReturnValue)
{
  typedef LogWriteBuf<MMapAnon> buf_type;
  auto sp = std::make_shared<buf_type>(MemManager::GetPageSize(), 0, -1, false, true);

  logstreambuf<buf_type> csb(sp);

  // sputn internally calls xsputn; xsputn returns Push() == 0 (success code, not byte count).
  // Document the actual behaviour so any change to the return value is caught.
  std::streamsize result = csb.sputn("testdata", 8);
  EXPECT_EQ(result, 0); // xsputn returns Push result (0 = success), not bytes written
}
