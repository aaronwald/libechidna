  ...
  private:
                             ...
                             size_t inlen, int final)
    {
      ...
      for (;;)
      {
        ...
        {
          ECHIDNA_LOG_DEBUG(_logger, "{0} = {1}", std::string(reinterpret_cast<char *>(nv.name), nv.namelen), std::string(reinterpret_cast<char *>(nv.value), nv.valuelen));
          ...
          if (::strncmp(reinterpret_cast<char *>(nv.name), ":path", std::min(nv.namelen, 5ul)) == 0)
          {
            ...
            {
              ECHIDNA_LOG_INFO(_logger, "Path {0}", std::string(reinterpret_cast<char *>(nv.value), nv.valuelen));
              con->_lastPathMatch = true;
            }
            else
            {
              ECHIDNA_LOG_ERROR(_logger, "Path mismatch {0}", std::string(reinterpret_cast<char *>(nv.value), nv.valuelen));
    ...
    {
      ECHIDNA_LOG_DEBUG(_logger, "SendFrame fd[{0}] type[{1}] flags[{2}] id[{3}] len[{4}]", con->_fd, (int)hdr.type, (int)hdr.flags, hdr.id, len);
